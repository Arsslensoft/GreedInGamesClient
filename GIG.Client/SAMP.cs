...
namespace GIG.Client
{
    public class SAMP
    {
        public static bool StartGame(string host, string port, string nickname, string password)
        {
            try
            {
                var registry = Registry.CurrentUser;

                RegistryKey registryKey = registry.OpenSubKey(@"SOFTWARE\\SAMP", true);
                registryKey.SetValue("PlayerName", nickname);
                registryKey.Close();

                string f =@"C:\Program Files (x86)\Rockstar Games\GTA San Andreas\samp.exe";
                if (File.Exists(f))
                {
 
                    if (password.Length > 0)
                        Process.Start(f, host + ":" + port + " " + password);
                    else
                        Process.Start(f, host + ":" + port);

                    return true;
                }
                else
                {
                    f = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles) + "\\Rockstar Games\\GTA San Andreas\\samp.exe";
                    if (File.Exists(f))
                    {
                        if (password.Length > 0)
                            Process.Start(f, host + ":" + port + " " + password);
        ...
        {
            Query sQuery = new Query(host, port);
    ...
    class Query
    {
        ...
        public bool SendA(string opcode)
        {
            try
            {
                EndPoint endpoint = new IPEndPoint(address, _port);
